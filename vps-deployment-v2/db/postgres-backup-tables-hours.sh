...
# Version 1.3 - Elias Beranbé Turchiello - 02/08/2023
...
FILES[logs]="${DIRECTORIES[logs]}/postgres-backup-tables-hours.log"


Database="nome-do-banco-de-dados"
TABLES_NOT_INCLUDED=('tabela-exluida-1' 'tabela-exluida-2' 'tabela-exluida-3' 'tabela-exluida-4') # List of all excluded tables
...
if [ $(pidof -x $(basename $0) | wc -w) -ge 3 ]; then
	echo -e "\nTHE HOURS BACKUP IS ALREADY RUNNING \nABORTING NOW... \n" | tee -a ${FILES[logs]}
	exit 1
fi
...
# LIST OF TABLES FROM THIS DATABASE
...
# EXCLUDE THE LIST OF TABLES CONFIGURED
for ((i=0; i<= ${#TABLES_LIST[@]}; i++)); do
	for tb in ${TABLES_NOT_INCLUDED[@]}; do
		if echo "${TABLES_LIST[$i]}" | tr -d \| | grep $tb &> /dev/null ; then
			TABLES_LIST[$i]=""
		fi
	done
done
...
ThisDirectory="${DIRECTORIES[dest]}/H-$DateTime-$Database"
