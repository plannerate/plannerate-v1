...
type SocialiteProvider = { provider: string; label: string };

const page = usePage<{ tenant?: { socialite_providers?: SocialiteProvider[] } }>();

const socialiteProviders = computed<SocialiteProvider[]>(
    () => page.props.tenant?.socialite_providers ?? [],
);

function socialiteRedirectUrl(provider: string): string {
...
<template>
    ...
    <AuthLayout :title="t('app.auth.login_title')">
        ...
        <!-- SSO providers (only when configured for this tenant) -->
        <template v-if="socialiteProviders.length > 0">
            ...
            <div class="flex flex-col gap-2.5">
                <a
                    v-for="p in socialiteProviders"
                    :key="p.provider"
                    :href="socialiteRedirectUrl(p.provider)"
                    class="flex w-full items-center justify-center gap-3 rounded-lg border border-border bg-background px-4 py-2.5 text-sm font-medium text-foreground transition hover:bg-muted/50"
                >
                    <!-- Google -->
                    <svg
                        v-if="p.provider === 'google'"
