...
const cloudflareCreateHref = computed(() =>
    isEdit.value && props.tenant?.host
        ? TenantCloudflareController.store
              .url(props.tenant!.id)
              .replace(/^\/\/[^/]+/, '')
        : '',
);
...
const cloudflareDeleteHref = computed(() =>
    isEdit.value && props.tenant?.host
        ? TenantCloudflareController.destroy
              .url(props.tenant!.id)
              .replace(/^\/\/[^/]+/, '')
        : '',
);
...
const pageMeta = useCrudPageMeta({
    headTitle: isEdit.value
        ? t('app.landlord.tenants.actions.edit')
    ...
    breadcrumbs: [
        ...
        },
        {
            ...
            href: isEdit.value
                ? tenantWayfinderPath(
                      TenantController.edit.url(props.tenant!.id),
                  )
                : tenantWayfinderPath(TenantController.create.url()),
        },
    ],
});
...
const alreadyRunning = computed(
    () =>
        props.tenant?.status === 'provisioning' &&
        !props.tenant?.provisioning_error,
);
...
function restartProvision(): void {
    provisionForm.post(
        tenantWayfinderPath(TenantController.provision.url(props.tenant!.id)),
    );
}
                ...
                >
                    ...
                    >
                        ...
                        <div class="flex-1 text-sm">
                            <span
                                v-if="tenant!.provisioning_error"
                            ...
                            >
                                Este tenant ainda não foi provisionado (status:
                        ...
                        <div class="flex shrink-0 items-center gap-2">
                            ...
                            <Link
                                :href="
                                    tenantWayfinderPath(
                                        TenantController.setup.url(tenant!.id),
                                    )
                                "
                    ...
                    <div class="grid gap-4 md:grid-cols-2">
                        <div class="grid gap-2">
                            ...
                            <select
                                ...
                                class="h-10 w-full rounded-md border border-input bg-background px-3 text-sm text-foreground transition outline-none focus:border-primary/60 focus:ring-2 focus:ring-primary/20"
                            >
                                <option
                    ...
                    <div class="grid gap-2">
                        <Label>{{
                            t('app.landlord.tenants.fields.modules')
                        }}</Label>
                        <div
                        ...
                        <div v-else class="grid gap-2 md:grid-cols-2">
                            ...
                            >
                                <input
                                    ...
                                    :value="module.id"
                                    :checked="
                                        props.tenant?.module_ids.includes(
                                            module.id,
                                        ) ?? false
                                    "
                    ...
                    <div class="grid gap-4">
                        <div class="grid gap-2">
                            ...
                            <Input
                                id="host"
                                name="host"
                                :default-value="props.tenant?.host ?? ''"
                                required
                            />
                    ...
                    >
                        ...
                        <div>
                            <span class="text-sm font-medium">{{
                                t(
                                    'app.landlord.tenants.fields.domain_is_active',
                                )
                            }}</span>
                            <p class="text-xs text-muted-foreground">
                                Ativa o domínio primário do tenant
                                imediatamente.
                            </p>
                        </div>
