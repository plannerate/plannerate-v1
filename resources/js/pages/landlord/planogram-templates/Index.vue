...
const pageMeta = useCrudPageMeta({
    headTitle: t('app.landlord.planogram_templates.title'),
    title: t('app.landlord.planogram_templates.title'),
    description: t('app.landlord.planogram_templates.description'),
    breadcrumbs: [
...
function handleDelete(id: string): void {
    router.delete(GlobalPlanogramTemplateController.destroy.url(id));
}
</script>
...
<template>
    <AppLayout :breadcrumbs="pageMeta.breadcrumbs" :page-header="pageMeta">
        ...
        <ListPage
            ...
            :search-placeholder="t('app.landlord.planogram_templates.search_placeholder')"
            :filter-label="t('app.landlord.planogram_templates.filter_label')"
            :clear-label="t('app.landlord.planogram_templates.clear_label')"
        >
            <table class="w-full text-sm">
                <thead class="bg-muted/30 text-left text-muted-foreground">
                    <tr>
                        <th class="px-4 py-3 font-medium">{{ t('app.landlord.planogram_templates.fields.code_name') }}</th>
                        <th class="px-4 py-3 font-medium">{{ t('app.landlord.planogram_templates.fields.department') }}</th>
                        <th class="px-4 py-3 font-medium">{{ t('app.landlord.planogram_templates.fields.subtemplates') }}</th>
                        <th class="px-4 py-3 font-medium">{{ t('app.landlord.planogram_templates.fields.products') }}</th>
                        <th class="px-4 py-3 font-medium">{{ t('app.landlord.planogram_templates.fields.shared_with') }}</th>
                        <th class="px-4 py-3 font-medium">{{ t('app.landlord.planogram_templates.fields.status') }}</th>
                        <th class="px-4 py-3 font-medium">{{ t('app.landlord.planogram_templates.fields.created_at') }}</th>
                        <th class="px-4 py-3 font-medium">{{ t('app.landlord.planogram_templates.fields.actions') }}</th>
                    </tr>
                </thead>
                <tbody>
                    ...
                    <tr v-else-if="templatesRows.length === 0">
                        <td class="px-4 py-8 text-center text-muted-foreground" colspan="8">
                            {{ t('app.landlord.planogram_templates.empty') }}
                            <a :href="GlobalPlanogramTemplateController.create.url()" class="ml-1 text-primary underline">
                                {{ t('app.landlord.planogram_templates.empty_action') }}
                            </a>
                        </td>
                    </tr>
                    <tr
                    ...
                    >
                        ...
                        <td class="px-4 py-3">
                            <Badge v-if="template.shares_count > 0" variant="secondary">
                                {{ template.shares_count }}
                            </Badge>
