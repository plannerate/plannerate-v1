 ...
 * - Zoom and pan controls
 -->
<template>
    <div :data-invalid="hasError" class="flex flex-col gap-y-1">
        ...
        <p v-if="column.helpText" class="text-sm text-muted-foreground">
            {{ column.helpText }}
        </p>
        ...
        <input
            v-if="mapImage && isNewImage"
            type="hidden"
            :name="`${column.name}[image]`"
            :value="mapImage"
        />
        <input
            v-if="mapImage"
            type="hidden"
            :name="`${column.name}[regions]`"
            :value="regionsJson"
        />
    </div>
</template>
...
const generateDuplicatedLabel = (sourceLabel?: string | null): string => {
