<template>
  <div
    ...
    <Button type="button" variant="ghost" size="icon" class="h-8 w-8" @click="$emit('zoom-out')" title="Diminuir zoom">
      <Minus class="h-4 w-4" />
    </Button>
    <span class="text-xs font-medium min-w-[50px] text-center">{{ Math.round(zoom * 100) }}%</span>
    <Button type="button" variant="ghost" size="icon" class="h-8 w-8" @click="$emit('zoom-in')" title="Aumentar zoom">
      <Plus class="h-4 w-4" />
    </Button>
    ...
    <Button 
      type="button" 
      :variant="currentTool === 'draw' && drawShape === 'rectangle' ? 'secondary' : 'ghost'"
      size="icon" 
      class="h-8 w-8" 
      @click="$emit('tool-change', 'draw'); $emit('shape-change', 'rectangle')"
    ...
    <Button 
      type="button" 
      :variant="currentTool === 'draw' && drawShape === 'circle' ? 'secondary' : 'ghost'"
      size="icon" 
      class="h-8 w-8" 
      @click="$emit('tool-change', 'draw'); $emit('shape-change', 'circle')" 
