        ...
        <ScrollArea class="flex-1" :style="maxHeight ? { maxHeight: `${maxHeight}px` } : undefined">
            <div class="p-2 space-y-1">
                <div v-for="region in regions" :key="region.id"
                    ...
                    :class="{ 'bg-muted': selectedRegionId === region.id }" @click="$emit('select', region)"
                    @dblclick="$emit('edit', region)">
                    <div class="w-3 h-3 rounded shrink-0"
                    ...
                    <div class="flex items-center gap-1 shrink-0">
                        <Button type="button" variant="ghost" size="icon" class="h-6 w-6" title="Duplicar área"
                            @click.stop="$emit('duplicate', region)">
                            <Copy class="h-3 w-3" />
                        </Button>
                        <Button type="button" variant="ghost" size="icon" class="h-6 w-6" title="Editar área"
...
const regionTypeLabel = (type?: string | null) => {
