...
return new class extends Migration
{
    ...
    {
        Schema::create('global_planogram_template_slots', function (Blueprint $table): void {
            $table->ulid('id')->primary();
            $table->ulid('subtemplate_id')->comment('FK → global_planogram_subtemplates');
            $table->unsignedTinyInteger('module_number')->comment('Módulo — col E');
            $table->unsignedTinyInteger('shelf_order')->comment('Ordem lógica (1=chão, crescente para cima) — col F');
            ...
            $table->timestamps();
            $table->softDeletes();
            $table->index(['subtemplate_id', 'module_number', 'shelf_order']);
        });
    }

    public function down(): void
    {
        Schema::dropIfExists('global_planogram_template_slots');
    }
};
