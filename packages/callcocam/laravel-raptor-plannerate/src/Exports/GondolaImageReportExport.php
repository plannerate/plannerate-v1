...
class GondolaImageReportExport
{
    ...
    private function createSpreadsheet(): Spreadsheet
    {
        ...
        $title = 'Image - '.$gondolaName;
        // Truncar se necessário para caber em 31 caracteres
        if (strlen($title) > 31) {
            $maxGondolaLength = 31 - strlen('Image - ');
        ...
        // Criar cabeçalhos específicos para image
        $this->createHeaders($sheet);

        // Preencher dados (filtrados para produtos da biblioteca sem imagem)
        $this->fillData($sheet);
    ...
    private function createSummary($sheet): void
    {
        ...
        // Filtrar produtos da biblioteca sem imagem para o resumo
        $libraryProductsWithoutImage = $this->filterProductsWithoutImage($products);
        ...
        $sheet->setCellValue('D5', 'Produtos Biblioteca s/ Img:');
        $sheet->mergeCells('D5:E5');
        // Contar produtos da biblioteca sem imagem
        $libraryProductsWithoutImageCount = count($libraryProductsWithoutImage);
        $sheet->setCellValue('F5', $libraryProductsWithoutImageCount);
    }

    /**
     * Cria os cabeçalhos específicos para relatório de image
     */
     ...
     */
    private function hasImage($product): bool
    {
        return isset($product['has_image']) && $product['has_image'] === true;
    }

    /**
     * Filtra produtos da biblioteca que não têm imagem
     */
    private function filterProductsWithoutImage($products): array
    {
        return array_filter($products, function ($product) {
            return ($product['source'] ?? '') === 'library' && ! ($product['has_image'] ?? false);
        });
    }

    /**
     * Preenche os dados na planilha (apenas produtos da BIBLIOTECA SEM imagem) - OTIMIZADO
     */
    private function fillData($sheet): void
    {
        ...
        // Filtrar apenas produtos da BIBLIOTECA SEM imagem
        $libraryProductsWithoutImage = $this->filterProductsWithoutImage($products);

        // Agrupar produtos por código ERP para evitar duplicatas
        $groupedProducts = [];
        foreach ($libraryProductsWithoutImage as $product) {
            ...
            if (! isset($groupedProducts[$key])) {
                $groupedProducts[$key] = [
                    ...
                    'tem_imagem' => 'Não', // Todos serão "Não" já que estamos filtrando
                ];
            }
        }
    ...
    {
        // Contar produtos únicos da BIBLIOTECA SEM imagem agrupados
        $products = $this->reportData['products'] ?? [];
        $libraryProductsWithoutImage = $this->filterProductsWithoutImage($products);
        $groupedProducts = [];
        foreach ($libraryProductsWithoutImage as $product) {
    ...
    private function setColumnWidths($sheet): void
    {
        $widths = [
            'A' => 15, // Código ERP
            'B' => 15, // EAN
            'C' => 30, // Descrição (nome) - reduzida para dar espaço
