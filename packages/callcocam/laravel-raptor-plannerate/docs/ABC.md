Sub AnaliseCompletaSortimentoEStatus()
    ...
    Dim tempCorteA As String
    Dim tempCorteB As String
    ...
    pesoQtde = configWs.Range("B2").Value
    pesoValor = configWs.Range("B3").Value
    pesoMargem = configWs.Range("B4").Value

    tempCorteA = Replace(configWs.Range("E2").Value, "<=", "")
    tempCorteA = Replace(tempCorteA, " ", "")
    corteA = Val(tempCorteA)

    tempCorteB = Replace(configWs.Range("E3").Value, "<=", "")
    tempCorteB = Replace(tempCorteB, " ", "")
    corteB = Val(tempCorteB)
    ...
    ' >>> Classificação ABC
    For i = 2 To ultimaLinha
        If ws.Cells(i, "M").Value <= 0.8 Then
    ...
    ' >>> Status do Produto (Agora correto e com regra do estoque)
    For i = 2 To ultimaLinha
        ...
        If ultimaVenda <> 0 And dataHoje - ultimaVenda <= 120 And ultimaCompra <> 0 And dataHoje - ultimaCompra <= 120 Then
            ws.Cells(i, "Q").Value = "Ativo"
            ws.Cells(i, "S").Value = "Venda e compra recentes"
        ElseIf ultimaVenda <> 0 And dataHoje - ultimaVenda <= 120 And (ultimaCompra = 0 Or dataHoje - ultimaCompra > 120) Then
            ws.Cells(i, "Q").Value = "Ativo"
            ws.Cells(i, "S").Value = "Venda recente, sem compra"
        ElseIf ultimaCompra <> 0 And dataHoje - ultimaCompra <= 120 And (ultimaVenda = 0 Or dataHoje - ultimaVenda > 120) Then
            ws.Cells(i, "Q").Value = "Ativo"
