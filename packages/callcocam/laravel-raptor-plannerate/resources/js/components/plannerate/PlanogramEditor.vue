...
/**
 * Editor de planograma unificado.
 *
 * Substitui a duplicação entre Planogram.vue (modo manual, com painel de
 * produtos) e PlanogramAuto.vue (modos template/automático, com painel de
 * geração). A prop `mode` decide o painel esquerdo:
 *
 * - 'manual'    → PanelLeft (busca/arrasto de produtos) + provides reais
 * - 'generated' → PanelLeftGeneration (regerar/redistribuir/reordenar +
 *                 overrides por categoria) + provides no-op
 *
 * Os wrappers Planogram.vue e PlanogramAuto.vue continuam existindo para
 * compatibilidade com os imports do app (Plannerate.vue decide qual usar
 * pelo generation_mode da gôndola).
 */
...
interface Props {
    /** 'manual' = painel de produtos; 'generated' = painel de geração */
...
/** Chave de localStorage do painel esquerdo varia conforme o modo (compatível com o legado). */
const leftPanelStorageKey = props.mode === 'manual'
...
// ── provides de produtos ──────────────────────────────────────────────────────
// Modo manual: function-refs reais vindas do PanelLeft.
// Modo generated: no-ops para compatibilidade com PanelRight/Shelf que injetam.
...
// ── painéis (persistidos em localStorage, chaves compatíveis com o legado) ────
...
const openLeftPanel = () => {
    ...
    if (isBrowser) {
        window.localStorage.setItem(leftPanelStorageKey, 'true');
    }
};

const closeLeftPanel = () => {
    storedLeftPanel.value = false;

    if (isBrowser) {
        window.localStorage.setItem(leftPanelStorageKey, 'false');
    }
};

const toggleLeftPanel = () => {
    storedLeftPanel.value = !storedLeftPanel.value;

    if (isBrowser) {
        window.localStorage.setItem(
            leftPanelStorageKey,
            storedLeftPanel.value ? 'true' : 'false',
        );
    }
};
...
onMounted(() => {
    ...
    if (isBrowser) {
        const stored = window.localStorage.getItem(leftPanelStorageKey);

        // Painel de geração abre por padrão na primeira visita; o de produtos não
        storedLeftPanel.value = stored === null
...
<template>
    <div class="h-screen overflow-hidden bg-background">
        ...
        <div class="relative flex overflow-hidden" :style="{ height: `${containerHeight}px` }">
            ...
            <!-- Painel esquerdo conforme o modo -->
            <PanelLeft v-if="isManual" :open="storedLeftPanel" :gondola-id="record.id"
                :planogram-id="record.planogram_id" :subdomain="resolvedSubdomain" :category="category"
                @close="closeLeftPanel" @reload-function="setReloadFunction"
                @remove-used-product="setRemoveUsedProductFunction" />
            <PanelLeftGeneration v-else :open="storedLeftPanel" :gondola="record" @close="closeLeftPanel" />

            <Canvas :record="record" :containerHeight="containerHeight" :openProperties="storedPropertiesPanel"
                :openProducts="storedLeftPanel" :saveChangesRoute="saveChangesRoute"
                @openProperties="openProperties" @closeProperties="closeProperties" @openProducts="openLeftPanel"
                @closeProducts="closeLeftPanel" />
            ...
            <button
                ...
                type="button" @click="toggleLeftPanel" v-if="!storedLeftPanel">
                ...
                <span
                    ...
                    {{ isManual ? t('plannerate.planogram.open_products') : t('plannerate.sidebar.generation.title') }}
                </span>
            </button>
