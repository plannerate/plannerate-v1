...
const currentRuleLabel = computed(() => {
    ...
    return rule?.label || t('plannerate.analysis.bcg_params.invalid_combination');
});
    ...
    <Dialog :open="open" @update:open="handleOpenChange">
        <DialogContent class="z-[1000] max-h-[90vh] max-w-2xl overflow-y-auto">
            ...
            <form @submit.prevent="handleSubmit" class="space-y-4">
                ...
                <div class="space-y-3">
                    <p class="text-xs font-semibold text-foreground">{{ t('plannerate.analysis.bcg_params.hierarchy_title') }}</p>

                    <!-- Atalho: regra pré-definida via native select -->
                    <div class="space-y-1.5">
                        <Label class="text-[10px] text-muted-foreground">{{ t('plannerate.analysis.bcg_params.analysis_rule') }}</Label>
                        <select
                    ...
                    <div class="grid grid-cols-2 gap-3 border-t pt-3">
                        <div class="space-y-1.5">
                            <Label class="text-[10px] text-muted-foreground">{{ t('plannerate.analysis.bcg_params.classify_by') }}</Label>
                            <select
                        ...
                        <div class="space-y-1.5">
                            <Label class="text-[10px] text-muted-foreground">{{ t('plannerate.analysis.bcg_params.display_by') }}</Label>
                            <select
                    ...
                    <div class="flex items-center gap-2 text-[10px]">
                        ...
                        <span :class="isCurrentCombinationValid ? 'text-green-700' : 'text-red-600'">
                            {{ isCurrentCombinationValid
                                ? t('plannerate.analysis.bcg_params.valid_combination')
                                : t('plannerate.analysis.bcg_params.invalid_combination') }}
                        </span>
                    </div>

                    <!-- Preview -->
                    <div class="rounded-md bg-muted/50 px-3 py-2 text-[10px] text-muted-foreground space-y-0.5">
                        <p class="font-medium text-foreground text-xs mb-1">{{ t('plannerate.analysis.bcg_params.analysis_preview') }}</p>
                        <p><strong>{{ t('plannerate.analysis.bcg_params.grouping') }}:</strong> {{ LEVEL_LABELS[form.classify_by] }}</p>
                        <p><strong>{{ t('plannerate.analysis.bcg_params.detail') }}:</strong> {{ LEVEL_LABELS[form.display_by] }}</p>
                        <p><strong>{{ t('plannerate.analysis.bcg_params.x_axis') }}:</strong> {{ form.x_axis }}</p>
                        <p><strong>{{ t('plannerate.analysis.bcg_params.y_axis') }}:</strong> {{ form.y_axis }}</p>
                    </div>
                </div>
                ...
                <div class="space-y-1 border-t pt-3">
                    <p class="text-xs font-semibold text-foreground">{{ t('plannerate.analysis.bcg_params.current_period') }}</p>
                ...
                <!-- Período Anterior -->
                <div class="space-y-1 border-t pt-3">
                    <p class="text-xs font-semibold text-foreground">{{ t('plannerate.analysis.bcg_params.previous_period') }}</p>
                    <p class="text-[10px] text-muted-foreground">{{ t('plannerate.analysis.bcg_params.previous_period_hint') }}</p>
                ...
                <div class="space-y-1.5 border-t pt-3">
                    <Label class="text-xs">{{ t('plannerate.analysis.bcg_params.growth_threshold') }}</Label>
                    <Input
                    ...
                    <p class="text-[10px] text-muted-foreground">
                        {{ t('plannerate.analysis.bcg_params.growth_threshold_hint') }}
                    </p>
                </div>
