...
class DiscoverIntegrationStockJob implements ShouldQueue
{
    ...
    {
        return "Estoque Visão: {$this->client->name} - {$this->store->name}";
    }
    ...
    public function handle(IntegrationSyncRetryService $retryService): void
    {
        ...
        try {
            ...
            Log::info('Iniciando descoberta de estoque Visão', [
            ...
            Log::info('Descoberta de estoque Visão concluída', [
                'client' => $this->client->name,
                'store' => $this->store->name,
            ...
            Log::error('Erro ao conectar com a API Visão (estoque): '.$e->getMessage(), [
                'client' => $this->client->name,
                'store' => $this->store->name,
                'trace' => $e->getMessage(),
            ]);
