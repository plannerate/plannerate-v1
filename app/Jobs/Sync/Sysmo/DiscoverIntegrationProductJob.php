...
class DiscoverIntegrationProductJob implements ShouldQueue
{
    ...
    {
        return "Produtos Sysmo: {$this->client->name} - {$this->store->name}";
    }
    ...
    public function handle(IntegrationSyncRetryService $retryService): void
    {
        ...
        try {
            ...
            Log::info('Iniciando descoberta de produtos Sysmo', [
            ...
            Log::info('Descoberta de produtos Sysmo concluída', [
                'client' => $this->client->name,
                'store' => $this->store->name,
            ...
            Log::error('Erro ao conectar com a API Sysmo (produtos): '.$e->getMessage(), [
                'client' => $this->client->name,
                'store' => $this->store->name,
                'trace' => $e->getMessage(),
            ]);
