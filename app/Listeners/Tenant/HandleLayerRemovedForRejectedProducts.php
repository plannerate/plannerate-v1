...
class HandleLayerRemovedForRejectedProducts
{
    ...
    public function handle(LayerRemovedEvent $event): void
    {
        ...
        if (! $product) {
            Log::warning('HandleLayerRemovedForRejectedProducts: produto não encontrado', [
        ...
        Log::info('✅ Produto inserido em rejeitados após remoção manual de layer', [
            'product_id' => $product->id,
            'gondola_id' => $event->gondola->id,
            'mode' => $event->gondola->generation_mode,
        ]);
    }
