...
/**
 * Notificação de conclusão do sync:cleanup (database + broadcast).
 * Usada pelo CleanupCommand para enviar resultados aos usuários.
 */
class CleanupCompletedNotification extends Notification implements ShouldBroadcastNow
{
    use Queueable;
    ...
    public function toArray(object $notifiable): array
    {
        return [
            ...
            'title' => $this->preview
                ? 'Sync Cleanup (preview) concluído'
    ...
    private function buildMessage(): string
    {
        $clientsSummary = collect($this->results)->map(function (array $r) {
            ...
            if (($r['restore_sold'] ?? 0) > 0) {
                $parts[] = "{$r['restore_sold']} produtos restaurados";
            }
