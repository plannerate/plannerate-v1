...
final readonly class CategoryAbcSummary
{
    public function __construct(
        ...
        /**
         * Soma das larguras dos produtos da subcategoria (em cm).
         * Usado pelo SlotPlanBuilder para calcular a demanda real de prateleiras.
         * Widths inválidas (≤ 0 ou > 60 cm) recebem fallback de 10 cm.
         */
        public float $totalWidth = 0.0,
    ) {}
     ...
     */
    /**
     * Limites para resolução de largura inline (mesma lógica do ProductWidthResolver).
     * Evita DI em método estático de DTO.
     */
    private const WIDTH_FALLBACK_CM = 10.0;

    private const WIDTH_MAX_PLAUSIBLE_CM = 60.0;
    ...
    public static function fromScoredProducts(string $categoryId, Collection $products, array $abcClassMap = []): self
    {
        ...
        foreach ($products as $sp) {
            ...
            $totalWidth += self::resolveWidth($sp->product);
    ...
    /**
     * Resolve a largura de um produto aplicando as mesmas regras do ProductWidthResolver:
     * valor inválido (≤ 0 ou > 60 cm) retorna o fallback de 10 cm.
     *
     * @param  mixed  $product  Objeto com propriedade `width` (nullable).
     */
    private static function resolveWidth(mixed $product): float
    {
        $raw = isset($product->width) ? (float) $product->width : 0.0;

        if ($raw <= 0 || $raw > self::WIDTH_MAX_PLAUSIBLE_CM) {
            return self::WIDTH_FALLBACK_CM;
        }
     ...
     * Retorna cópia com totalQuantity e totalMargem substituídos pelos valores normalizados (0–1).
     * Passar ao CategoryRoleInferrer, que espera participação relativa — não somas brutas.
     * O DTO original (com somas brutas e totalWidth) deve ser passado ao SlotPlanBuilder.
     */
    public function withParticipation(float $normalizedQuantity, float $normalizedMargem): self
    {
        return new self(
            ...
            totalWidth: $this->totalWidth, // preserva largura real para o SlotPlanBuilder
        );
    }
}
